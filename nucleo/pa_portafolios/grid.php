	...
	$.ajax({
	 ...
	 $.ajax({
         ...
         url:  "../base/getdatossql.php?bd=Mysql&sql="+encodeURI("select e.ALUCTR as MATRICULA,e.PDOCVE AS CICLO, e.MATCVE AS MATERIA, f.MATE_DESCRIP AS MATERIAD, "+
                 " e.GPOCVE AS GRUPO,"+
                 " IFNULL((SELECT RUTA FROM eadjuntos b where b.ID=CONCAT(PDOCVE,ALUCTR,MATCVE) and b.AUX='ENCUADRE'),'') AS RUTAENCUADRE, "+
                 " IFNULL((SELECT RUTA FROM eadjuntos b where b.ID=CONCAT(PDOCVE,ALUCTR,MATCVE) and b.AUX='DIAGNOSTICA'),'') AS RUTADIAGNOSTICA "+     
      ...
      function subirUnidades(matricula, ciclo, id, materia, descrip){
		    ...
		    $.ajax({
		           ...
		           url:  "../base/getdatossql.php?bd=Mysql&sql=SELECT UNID_ID, UNID_NUMERO, UNID_DESCRIP, "+
		                  "IFNULL((SELECT RUTA FROM eadjuntos b where b.ID=CONCAT(UNID_ID,'"+ciclo+matricula+materia+"') and b.AUX='EP'),'') AS RUTAEP, "+
		                  "IFNULL((SELECT RUTA FROM eadjuntos b where b.ID=CONCAT(UNID_ID,'"+ciclo+matricula+materia+"') and b.AUX='ED'),'') AS RUTAED, "+
		                  "IFNULL((SELECT RUTA FROM eadjuntos b where b.ID=CONCAT(UNID_ID,'"+ciclo+matricula+materia+"') and b.AUX='EC'),'') AS RUTAEC, "+
		                  "IFNULL((SELECT RUTA FROM eadjuntos b where b.ID=CONCAT(UNID_ID,'"+ciclo+matricula+materia+"') and b.AUX='EA'),'') AS RUTAEA "+
