...
<html lang="es">
	...
	<div class="preloader-wrapper"><div class="preloader"><img src="<?php echo $nivel; ?>imagenes/menu/preloader.gif"></div></div>
      ...
      <h3 class="header smaller lighter text-warning"><strong>Actividades en L&iacute;nea <i class="ace-icon fa fa-angle-double-right"></i> <small id="elciclo"></small> <small id="elciclod"></small></strong></h3>
...
<script type="text/javascript">
 ...
 function generaTabla(grid_data){
       ...
       jQuery.each(grid_data, function(clave, valor) { 	
    	    ...
    	    $("#row"+valor.ID).append("<td style= \"text-align: center;\" ><a  onclick=\"captActividades('"+valor.ID+"','<?php echo $_SESSION['usuario'];?>','"+valor.MATERIA+"','"+valor.MATERIAD+"','"+valor.SIE+"','"+valor.CICLO+"');\" title=\"Capturar actividades para la asignatura\" "+
                    "class=\"btn btn-white btn-waarning btn-bold\">"+
                    "<i class=\"ace-icon fa fa-list-alt  bigger-160 green \"></i>"+
                    "</a></td>");
    	    $("#row"+valor.ID).append("<td style= \"text-align: center;\" ><a  onclick=\"verTareas('"+valor.ID+"','<?php echo $_SESSION['usuario'];?>','"+valor.MATERIA+"','"+valor.MATERIAD+"','"+valor.SIE+"','"+valor.CICLO+"');\" title=\"Ver las actividades que han subido los alumnos\" "+
                    "class=\"btn btn-white btn-waarning btn-bold\">"+
                    "<i class=\"ace-icon fa fa-edit bigger-160 blue \"></i>"+
                    "</a></td>");
        });
...
function captActividades(id,profesor,materia,materiad,grupo,ciclo){
	 window.location="captActiv.php?&id="+id+"&ciclo="+ciclo+"&profesor="+profesor+"&grupo="+grupo+"&materia="+materia+"&materiad="+materiad+"&modulo=<?php echo $_GET["modulo"];?>";
}


function verTareas(id,profesor,materia,materiad,grupo,ciclo){
	 window.location="verActiv.php?&id="+id+"&ciclo="+ciclo+"&profesor="+profesor+"&grupo="+grupo+"&materia="+materia+"&materiad="+materiad+"&modulo=<?php echo $_GET["modulo"];?>";
}
