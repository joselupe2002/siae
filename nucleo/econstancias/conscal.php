...
/*===================================================================================================================*/
       ...
       // $data2 = $pdf->LoadDatosCursando($elciclo);
        ...
        $dataAlum[0]["PROMEDIO_SR"]. " CON UN AVANCE DEL ".explode("|",$dataAlum[0]["AVANCE"])[2]."%. CON LAS CALIFICACIONES QUE ".
        ...
        $pdf->Cell(150,5,'MATERIA',1,0,'C',true);
        $pdf->Cell(20,5,'CAL.',1,0,'C',true);
        $pdf->Ln();
        $pdf->SetFont('Montserrat-Medium','',10);
        $pdf->SetFillColor(172,31,6);
        $pdf->SetTextColor(0);
        $pdf->SetWidths(array(150,20));
        $n=1;
        foreach($data as $row) {
