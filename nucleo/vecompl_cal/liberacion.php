...
<?php session_start(); if (($_SESSION['inicio']==1)) {
   ...
   class PDF extends FPDF {
			...
			function LoadDatosEmpl($empl)
			{				
				$miConex = new Conexion();
				$resultado=$miConex->getConsulta($_SESSION['bd'],"SELECT * from pempleados where EMPL_NUMERO='".$empl."'");				
				foreach ($resultado as $row) {
					$data[] = $row;
				}
				return $data;
		...
		if (($_GET["tipo"]=='1')) {
			...
			$pdf->Image($dataR[0]["EMPL_FIRMA"],30,210,40);
