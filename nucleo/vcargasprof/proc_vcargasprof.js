	...
	if (table.rows('.selected').data().length>0) {
	    ...
	    $.ajax({
        	...
        	success: function(data){ 
				               ...
				               elsql="SELECT DESC_CLAVE, concat(DESC_CLAVE,' ',DESC_DESCRIP) FROM etipodescarga WHERE DESC_ACTIVO='S' ORDER BY DESC_DESCRIP";
...
function oficioDesSel(modulo,usuario,institucion, campus,essuper){	
	table = $("#G_"+modulo).DataTable();
	if (table.rows('.selected').data().length>0) {
		enlace="nucleo/vcargasprof/oficiodes.php?ID="+table.rows('.selected').data()[0][0]+"&ciclod="+table.rows('.selected').data()[0][3]+"&ciclo="+table.rows('.selected').data()[0][2]+"&tipo=1";
		abrirPesta(enlace, "OficioDes");	
	}
	else {
		alert ("Debe seleccionar un profesor");
