...
<?php session_start(); if (($_SESSION['inicio']==1)) {
   ...
   class PDF extends FPDF {
		...
		if (!($datades[0]==null)) { 
			...
			$pdf->Cell(0,0,'ASUNTO: DESCARGA ACADÉMICA.',0,1,'R');
			$pdf->SetFont('Montserrat-ExtraBold','B',9);
			$pdf->Ln(20);
			...
			$pdf->Ln(5);			
			$headerdes = array('CLAVE','ACTIVIDAD','DESCRIPCIÓN','HORAS');
			$pdf->imprimeDescarga($headerdes,$datades);
			$pdf->Ln(5);
			$pdf->SetFont('Montserrat-SemiBold','',10);
			$pdf->MultiCell(0,8,"Mismas que para su liberación quedarán sujetas a la entrega de las evidencias correspondientes, ".
					            "las cuáles deberán apegarse a los procedimientos e instructivo de trabajo alojados en el SGC del ".
					            "cual forma parte y que le fueron difundidos con anterioridad.",0,'J', false);
			$pdf->Ln(5);
			$pdf->MultiCell(0,8,"Sin más por el momento envió un cordial saludo.",0,'J', false);
