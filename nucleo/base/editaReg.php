...
<?php 
    ...
    session_start(); if (($_SESSION['inicio']==1)  && (strpos($_SESSION['permisos'],$_GET["modulo"])) ){ 
...
<body style="background-color: white;">
 ...
 <div class="panel panel-success">
    <div class="panel-heading"> <b><i class="ace-icon blue fa fa-pencil"></i> Editar Registro <?php echo $_GET["valorllave"];?></b></div>
	     ...
	     <div class="preloader-wrapper"><div class="preloader"><img src="<?php echo $nivel; ?>imagenes/menu/preloader.gif"></div></div> 
		     ...
		     <form style="width: 100%" method="post" id="frmReg" name="frmReg">
	               <fieldset>
                      ...
                      <?php 
                      $laTabla= $_GET['tabla']; 
                      $laTablaGraba= $_GET['tablagraba'];                         
                      $resDatos=$miConex->getConsulta($_GET['bd'],"SELECT ".$miUtil->getCamposFrm($_GET['tabla'],$_GET['bd'])." FROM ".$_GET['tabla']." WHERE ".$_GET['campollave']."='".$_GET['valorllave']."'");
                      
                      ?>
	                   <script type="text/javascript">
	                    bd="<?php echo $_GET["bd"]?>";
	                    datos=<?php echo json_encode($resDatos);?>;</script>
	                   
	                   <?php $misTabs=$miUtil->getTabs($_GET['modulo']); ?>
	                   <script type="text/javascript"> tabs=<?php echo json_encode($misTabs);?></script>
		...
		function cargarElementos(callback) {
			...
			claseActive="class=\"active\"";
		     ...
		     jQuery.each(campos, function(clave, valor){
                    ...
                    if (datos[0][valor.colum_name]==null) {elDato="";}else { elDato=datos[0][valor.colum_name];}  
	  ...
	  function guardar(){
			...
			if (!(campo=="")) {
				...
				$("#"+pes).addClass("in active");
				 ...
				 <?php echo $miUtil->getParamSubmit($rescampos,$laTablaGraba,"A",$_GET["bd"]);?>	
